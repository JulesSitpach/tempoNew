...
export class SAMGovService {
  ...
  static async searchEntitiesByName(entityName: string): Promise<any[]> {
    try {
      const response = await fetch(
        `${this.BASE_URL}?entityName=${encodeURIComponent(
          entityName
        )}&api_key=${API_KEYS.SAMGOV}`,
        {
          headers: {
            Accept: "application/json",
          },
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.log(`No SAM.gov results found for: ${entityName}`);
          return [];
        }
        throw new Error(`SAM.gov search error: ${response.status}`);
      }
      ...
      console.error(`SAM.gov entity search error for "${entityName}":`, error);
      return [];
    }
  }
}
...
export class SMBContextService {
  static async receiveUserContext(context: any): Promise<void> {
    // Stub implementation
    console.log("SMB context received:", context);
  }

  static async getSMBContext(sessionId: string): Promise<any> {
    // Stub implementation
    return null;
  }
}
