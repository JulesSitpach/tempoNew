...
// Shippo Service for shipping and logistics
export class ShippoService {
  private static readonly API_BASE = import.meta.env.DEV
    ? "/shippo-api/v1" // Use proxy in development
    : "https://api.goshippo.com/v1"; // Direct API in production
  private static readonly API_KEY = import.meta.env.VITE_SHIPPO_API_KEY;
  ...
  static async getCarriers(): Promise<any> {
    try {
      ...
      const response = await fetch(`${this.API_BASE}/carriers`, {
        method: "GET",
        headers: {
  ...
  static async validateAddress(address: any): Promise<any> {
    try {
      ...
      // Mock address validation for now
      return {
        status: "success",
        is_valid: true,
        ...address,
  ...
  static async getShippingRates(
    ...
    try {
      ...
      return {
        status: "error",
        message:
          error instanceof Error ? error.message : "Rate calculation failed",
        rates: [],
      };
    }
  }
...
// External API Status class for monitoring API health
export class ExternalAPIStatus {
  private static instance: ExternalAPIStatus;
  private apiStatuses: Map<
    string,
    { status: string; lastChecked: Date; responseTime?: number }
  > = new Map();

  private constructor() {}

  public static getInstance(): ExternalAPIStatus {
    if (!ExternalAPIStatus.instance) {
      ExternalAPIStatus.instance = new ExternalAPIStatus();
    }
    return ExternalAPIStatus.instance;
  }
  ...
  public getAllStatuses() {
    return Object.fromEntries(this.apiStatuses);
  }
}
