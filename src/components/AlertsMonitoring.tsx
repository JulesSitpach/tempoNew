...
const AlertsMonitoring = ({
  ...
  const generateDynamicAlerts = () => {
    ...
    if (productData.length > 0) {
      ...
      if (highImpactProducts.length > 0) {
        alerts.push({
          ...
          message: `${
            highImpactProducts.length
          } products from your uploaded data have tariff rates above ${
            alertSettings.tariffThreshold
          }%. Total impact: ${productData
            .reduce(
              (sum: number, p: any) =>
                sum +
                ((parseFloat(p.newCost) || 0) -
                  (parseFloat(p.originalCost) || 0)) *
    ...
    if (productData.length > 0) {
      ...
      alerts.push({
        ...
        message: `Cost Calculator data shows ${
          productData.length
        } products with ${avgTariffRate.toFixed(
          1
        )}% average tariff rate. Cross-module analysis suggests ${Math.floor(
          productData.length * 0.3
        )} products need supplier diversification.`,
  ...
  const checkAPIConfiguration = () => {
    ...
    Object.entries(apiKeys).forEach(([name, config]) => {
      ...
      if (name === "UN Comtrade") {
        const comtradeConfig = config as {
          primary: any;
          secondary: any;
          required: boolean;
          description: string;
